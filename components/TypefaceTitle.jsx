 ...
 * `enabled=false` mientras el boot overlay está: ni carga fuentes ni anima.
 */
const TypefaceTitle = React.memo(function TypefaceTitle({ text, className, enabled = true }) {
  const ref = useRef(null);
  const value = String(text ?? '');

  useTypefaceCycle(ref, HERO_CYCLE_FONTS, { text: value, enabled });
